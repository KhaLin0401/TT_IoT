...
var dbRef1 = firebase.database().ref('Area1').child('humiditySolid');
dbRef1.on('value', snap => humiditySolid.innerText = snap.val());

var humidityAir = document.getElementById('humidityAir_value');
var dbRef2 = firebase.database().ref('Area1').child('humidityAir');
dbRef2.on('value', snap => humidityAir.innerText = snap.val());

var light = document.getElementById('light_value');
var dbRef3 = firebase.database().ref('Area1').child('ldr');
dbRef3.on('value', snap => light.innerText = snap.val());

var temperature = document.getElementById('temperature_value');
var dbRef4 = firebase.database().ref('Area1').child('temperature');
dbRef4.on('value', snap => temperature.innerText = snap.val());

function changeArea2() {
  // Thực hiện các hành động mong muốn khi nhấp vào "AREA 2"
  var targetUrl = "Area2.html"; // Đường dẫn của trang web mục tiêu
  window.location.href = targetUrl; // Chuyển hướng đến trang web mục tiêu
}

//TOGGLE1
var switchCheckbox1 = document.getElementById('btn1');
const databaseRef = firebase.database().ref('Area1');
function toggle1() {
...
function updateValue() {
  ...
  var dbRef1 = database.ref('Area1').child('humiditySolid');
...
function updateValue2() {
  ...
  var dbRef1 = database.ref('Area1').child('humidityAir');
  dbRef1.once('value', function(snapshot) {
    var newValue = snapshot.val(); // Lấy giá trị từ snapshot
...
function updateValue3() {
  ...
  var dbRef3 = database.ref('Area1').child('ldr');
...
function updateValue4() {
  ...
  var dbRef4 = database.ref('Area1').child('temperature');
